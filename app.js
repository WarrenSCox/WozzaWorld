...
(function setupAdaptiveTripFooter(){
  ...
  function queueScan(){
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{scanQueued=false;scan()}));
  }
  function retryUnmeasured(){
    ...
    if(pending)setTimeout(()=>requestAnimationFrame(retryUnmeasured),90);
  }
  ...
  const mo=new MutationObserver(muts=>{if(muts.some(m=>!m.target.closest?.('.trip-flap-icons,.trip-card-flags'))){queueScan();setTimeout(retryUnmeasured,110)}});
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{queueScan();setTimeout(retryUnmeasured,140)});else{queueScan();setTimeout(retryUnmeasured,140)};
})();
