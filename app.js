...
const TRAVEL_MODES=['Air','Car','Campervan','Train','Ferry','Cruise','Narrowboat','Coach / Bus','Motorbike','Bicycle','On foot','Other'];
...
function travelModeOptions(selected=''){const normalized=String(selected||'').toLowerCase()==='sea'?'Ferry':selected;return `<option value="">Choose transport...</option>${TRAVEL_MODES.map(t=>`<option value="${t}"${t===normalized?' selected':''}>${t}</option>`).join('')}`}
