...
function attachCountryListSwipe(){
  ['visitedCountries','goingCountries','bucketCountries'].forEach(id=>{
    const list=$('#'+id);if(!list||list.dataset.cycleSwipe==='1')return;list.dataset.cycleSwipe='1';
    let sx=0,sy=0,tracking=false,swipedAt=0;
    list.addEventListener('touchstart',e=>{if(e.touches.length!==1)return;const t=e.touches[0];sx=t.clientX;sy=t.clientY;tracking=true},{passive:true});
    list.addEventListener('touchend',e=>{if(!tracking)return;tracking=false;const t=e.changedTouches[0],dx=t.clientX-sx,dy=t.clientY-sy;if(Math.abs(dx)>=55&&Math.abs(dx)>Math.abs(dy)*1.25){swipedAt=Date.now();setCountrySlide(countrySlide+(dx<0?-1:1),true,dx<0?'prev':'next')}},{passive:true});
    list.addEventListener('touchcancel',()=>tracking=false,{passive:true});
    list.addEventListener('click',e=>{if(Date.now()-swipedAt<450){e.preventDefault();e.stopImmediatePropagation()}},true);
  });
}
...
if(homeStatusSummary&&!homeStatusSummary.dataset.cycleSwipeReady){
  ...
  homeStatusSummary.addEventListener('pointerup',e=>{
    ...
    if(Math.abs(dx)>48&&Math.abs(dx)>Math.abs(dy)*1.2){
      setCountrySlide(countrySlide+(dx<0?-1:1),true,dx<0?'prev':'next');
    }
  });
...
/* Country list swiping is handled by #countryCarousel above to avoid duplicate swipe steps. */
;
