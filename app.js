...
const TRAVEL_MODES=['Air','Car','Train','Sea','Coach / Bus','Motorbike','Bicycle','On foot','Other'];
...
#tripDestinationStops>.trip-stop-mobile-source{display:block!important;opacity:.42!important;box-shadow:0 6px 14px rgba(0,35,55,.12)!important}
...
.trip-stop-mobile-ghost{transform:none!important;box-shadow:0 8px 18px rgba(0,35,55,.18)!important;opacity:.88!important;will-change:top}
`;document.head.appendChild(st)})();

;(()=>{
  if(window.__wozzaFastFactsImageGoogle)return;
  window.__wozzaFastFactsImageGoogle=true;
  document.addEventListener('click',e=>{
    const img=e.target.closest('img');
    if(!img)return;
    const facts=img.closest('#countryInfoDialog,#fastFactsDialog,.country-info-dialog,.fast-facts-dialog,.country-facts-modal');
    if(!facts)return;
    const hero=img.closest('.country-info-hero,.fast-facts-hero,.country-facts-hero,.facts-hero')||img.matches('.country-info-hero img,.fast-facts-hero img,.country-facts-hero img,.facts-hero img');
    if(!hero)return;
    const country=(facts.querySelector('[data-country-name],.country-name,.facts-country-name,h1,h2')?.textContent||currentCountry||'').trim();
    if(!country)return;
    ...
    else if(typeof openExternal==='function')openExternal(url);
