...
function attachCountryListSwipe(){
  ['visitedCountries','goingCountries','bucketCountries'].forEach(id=>{
    ...
    list.addEventListener('touchend',e=>{if(!tracking)return;tracking=false;const t=e.changedTouches[0],dx=t.clientX-sx,dy=t.clientY-sy;if(Math.abs(dx)>=55&&Math.abs(dx)>Math.abs(dy)*1.25){swipedAt=Date.now();setCountrySlide(countrySlide+(dx<0?1:-1),true,dx<0?'next':'prev')}},{passive:true});
