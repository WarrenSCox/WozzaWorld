...
function updateCountryLabels(transform){
 ...
 if(!portrait){
  ...
  labels.each(function(d){
   const pt=transform.apply(d.centroid);if(d.name==='Croatia'){pt[0]+=14;pt[1]+=16}const bw=(d.bounds[1][0]-d.bounds[0][0])*transform.k,bh=(d.bounds[1][1]-d.bounds[0][1])*transform.k,font=8,tw=Math.max(8,d.name.length*4.25);
 ...
 labels.each(function(d){
  ...
  if(d.name==='Croatia'){pt[0]+=14;pt[1]+=16}
...
const homeMapStage=$('#mapStage');if(homeMapStage){homeMapStage.setAttribute('role','button');homeMapStage.setAttribute('tabindex','0');homeMapStage.setAttribute('aria-label','Open World View map');homeMapStage.addEventListener('click',e=>{if(!document.body.classList.contains('map-view')){e.preventDefault();showMap()}});homeMapStage.addEventListener('keydown',e=>{if(!document.body.classList.contains('map-view')&&(e.key==='Enter'||e.key===' ')){e.preventDefault();showMap()}})}
