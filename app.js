...
function toast(t,action=null){const el=$('#toast'),tripConfirmation=t==='Trip updated ✓'||t==='Trip created ✈';el.textContent=t;if(tripConfirmation){el.style.background='rgba(49,67,94,.88)';el.style.color='#fff';el.style.opacity='1'}else{el.style.removeProperty('background');el.style.removeProperty('color');el.style.removeProperty('opacity')}el.classList.add('show');const clearTripStyle=()=>{el.style.removeProperty('background');el.style.removeProperty('color');el.style.removeProperty('opacity')};el.onclick=action?()=>{action();el.classList.remove('show');clearTripStyle()}:null;clearTimeout(el._timer);el._timer=setTimeout(()=>{el.classList.remove('show');el.onclick=null;clearTripStyle()},action?4200:1800)}
