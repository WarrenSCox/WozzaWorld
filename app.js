...
/* WozzaWorld custom calendar: visual picker only; stored dates remain YYYY-MM-DD. */
(()=>{
  if(document.getElementById('wozza-calendar-style'))return;
  const st=document.createElement('style');st.id='wozza-calendar-style';st.textContent=`
  .wozza-calendar-overlay{position:fixed;inset:0;z-index:2147483600;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(7,63,82,.24);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}
  .wozza-calendar{width:min(390px,calc(100vw - 28px));background:#f4fbfc;border-radius:24px;box-shadow:0 22px 60px rgba(7,63,82,.28);overflow:hidden;color:#073f52;font-family:inherit}
  .wozza-calendar-head{background:#073f52;color:white;padding:18px 22px 16px}.wozza-calendar-kicker{font-size:12px;font-weight:800;letter-spacing:.09em;text-transform:uppercase;opacity:.78}.wozza-calendar-picked{font-size:25px;font-weight:800;line-height:1.15;margin-top:5px}
  .wozza-calendar-nav{display:grid;grid-template-columns:44px 1fr 44px;align-items:center;padding:13px 14px 8px}.wozza-calendar-nav button{border:0;background:transparent;color:#073f52;font-size:28px;line-height:40px;border-radius:50%;cursor:pointer}.wozza-calendar-nav button:active{background:#dcebed}.wozza-calendar-month{text-align:center;font-size:17px;font-weight:800}
  .wozza-calendar-week,.wozza-calendar-grid{display:grid;grid-template-columns:repeat(7,1fr);padding:0 14px}.wozza-calendar-week span{text-align:center;font-size:11px;font-weight:800;color:#6f7e82;padding:8px 0}.wozza-calendar-grid{row-gap:4px}.wozza-calendar-day{height:42px;border:0;background:transparent;border-radius:50%;font:inherit;font-weight:700;color:#073f52;cursor:pointer}.wozza-calendar-day.outside{visibility:hidden}.wozza-calendar-day.today{box-shadow:inset 0 0 0 2px #056b89}.wozza-calendar-day.selected{background:#e9bd25;color:#17213D;box-shadow:none}.wozza-calendar-day:active{transform:scale(.94)}
...
function wozzaCalendarEnsure(){let ov=document.getElementById('wozzaCalendarOverlay');if(ov)return ov;ov=document.createElement('div');ov.id='wozzaCalendarOverlay';ov.className='wozza-calendar-overlay';ov.hidden=true;ov.innerHTML=`<div class="wozza-calendar" role="dialog" aria-modal="true" aria-label="Choose date"><div class="wozza-calendar-head"><div class="wozza-calendar-kicker">Choose date</div><div class="wozza-calendar-picked"></div></div><div class="wozza-calendar-nav"><button type="button" data-cal-prev aria-label="Previous month">‹</button><div class="wozza-calendar-month"></div><button type="button" data-cal-next aria-label="Next month">›</button></div><div class="wozza-calendar-week"><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span><span>Sun</span></div><div class="wozza-calendar-grid"></div><div class="wozza-calendar-actions"><button type="button" class="wozza-calendar-clear">Clear</button><button type="button" class="wozza-calendar-cancel">Cancel</button><button type="button" class="wozza-calendar-ok">OK</button></div></div>`;document.body.appendChild(ov);ov.querySelector('[data-cal-prev]').onclick=()=>{wozzaCalendarView=new Date(wozzaCalendarView.getFullYear(),wozzaCalendarView.getMonth()-1,1);wozzaCalendarRender()};ov.querySelector('[data-cal-next]').onclick=()=>{wozzaCalendarView=new Date(wozzaCalendarView.getFullYear(),wozzaCalendarView.getMonth()+1,1);wozzaCalendarRender()};ov.querySelector('.wozza-calendar-cancel').onclick=wozzaCalendarClose;ov.querySelector('.wozza-calendar-clear').onclick=()=>{wozzaCalendarSelected='';wozzaCalendarCommit()};ov.querySelector('.wozza-calendar-ok').onclick=wozzaCalendarCommit;ov.addEventListener('click',e=>{if(e.target===ov)wozzaCalendarClose()});return ov}
function wozzaCalendarRender(){const ov=wozzaCalendarEnsure(),view=wozzaCalendarView||new Date(),sel=wozzaDateFromIso(wozzaCalendarSelected),today=new Date();ov.querySelector('.wozza-calendar-month').textContent=view.toLocaleDateString('en-GB',{month:'long',year:'numeric'});ov.querySelector('.wozza-calendar-picked').textContent=sel?sel.toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'long',year:'numeric'}):'No date selected';const first=new Date(view.getFullYear(),view.getMonth(),1),days=new Date(view.getFullYear(),view.getMonth()+1,0).getDate(),offset=(first.getDay()+6)%7,grid=ov.querySelector('.wozza-calendar-grid');let html='';for(let i=0;i<offset;i++)html+='<span></span>';for(let d=1;d<=days;d++){const x=new Date(view.getFullYear(),view.getMonth(),d),iso=wozzaIsoDate(x),isToday=x.getFullYear()===today.getFullYear()&&x.getMonth()===today.getMonth()&&d===today.getDate();html+=`<button type="button" class="wozza-calendar-day${iso===wozzaCalendarSelected?' selected':''}${isToday?' today':''}" data-cal-date="${iso}">${d}</button>`}grid.innerHTML=html;grid.querySelectorAll('[data-cal-date]').forEach(b=>b.onclick=()=>{wozzaCalendarSelected=b.dataset.calDate;wozzaCalendarRender()})}
function wozzaCalendarOpen(input){wozzaCalendarTarget=input;const row=input.closest('.trip-destination-stop'),isEnd=input.classList.contains('trip-destination-to'),start=row?.querySelector('.trip-destination-from')?.value||'',existing=input.value||'',seed=existing||(isEnd?start:'')||wozzaIsoDate(new Date()),d=wozzaDateFromIso(seed)||new Date();wozzaCalendarSelected=existing;wozzaCalendarView=new Date(d.getFullYear(),d.getMonth(),1);const ov=wozzaCalendarEnsure();ov.hidden=false;wozzaCalendarRender();requestAnimationFrame(()=>ov.querySelector('.wozza-calendar-cancel')?.focus())}
function wozzaCalendarClose(){const ov=document.getElementById('wozzaCalendarOverlay');if(ov)ov.hidden=true;wozzaCalendarTarget=null}
...
function bindWozzaDateInputs(root=document){root.querySelectorAll?.('.trip-destination-from,.trip-destination-to').forEach(input=>{if(input.dataset.wozzaCalendarBound)return;input.dataset.wozzaCalendarBound='1';input.readOnly=true;input.addEventListener('click',e=>{e.preventDefault();wozzaCalendarOpen(input)});input.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();wozzaCalendarOpen(input)}})})}
