...
/* v0.18.xx — deterministic trip-card split-flap icon overflow */
function tripFlapIconItems(t){
  ...
  return [
    ...modes.map(m=>({label:m,html:travelModeIcon(m)})),
    ...vibes.map(v=>({label:v,html:vibeIcon(v)}))
  ];
}
function tripFlapIcons(t){
  const items=tripFlapIconItems(t), slots=4;
  if(!items.length)return '';
  const visible=items.slice(0,slots);
  return `<div class="trip-flap-icons${items.length>slots?' has-overflow':''}" data-trip-flap="${esc(t.id||'')}" data-flap-items="${encodeURIComponent(JSON.stringify(items))}" aria-label="Travel methods and trip vibes: ${esc(items.map(x=>x.label).join(', '))}">${visible.map((x,i)=>`<span class="trip-flap-slot" data-flap-slot="${i}" title="${esc(x.label)}">${x.html}</span>`).join('')}</div>`;
}
(function setupTripFlapSystem(){
  if(document.getElementById('wozza-trip-flap-style'))return;
  const st=document.createElement('style');st.id='wozza-trip-flap-style';st.textContent=`
    #tripList .trip-flap-icons{display:inline-flex;align-items:center;gap:4px;min-width:0;overflow:hidden;perspective:240px}
    ...
    #tripList .trip-flap-slot.flap-out{animation:wozzaFlapOut .16s ease-in forwards}
    #tripList .trip-flap-slot.flap-in{animation:wozzaFlapIn .20s ease-out forwards}
    @keyframes wozzaFlapOut{0%{transform:rotateX(0);opacity:1}100%{transform:rotateX(-88deg);opacity:.15}}
    @keyframes wozzaFlapIn{0%{transform:rotateX(88deg);opacity:.15}100%{transform:rotateX(0);opacity:1}}
    @media(prefers-reduced-motion:reduce){#tripList .trip-flap-slot.flap-out,#tripList .trip-flap-slot.flap-in{animation:none!important}}
  `;document.head.append(st);
  const states=new WeakMap();
  function init(row){
    if(states.has(row)||!row.classList.contains('has-overflow'))return;
    let items=[];try{items=JSON.parse(decodeURIComponent(row.dataset.flapItems||''))}catch(e){}
    if(items.length<=4)return;
    const state={offset:0,timer:null};states.set(row,state);
    const tick=()=>{
      if(!row.isConnected){clearInterval(state.timer);return}
      state.offset=(state.offset+4)%items.length;
      [...row.querySelectorAll('.trip-flap-slot')].forEach((slot,i)=>{
        const item=items[(state.offset+i)%items.length];
        setTimeout(()=>{
          slot.classList.remove('flap-in');slot.classList.add('flap-out');
          setTimeout(()=>{slot.innerHTML=item.html;slot.title=item.label;slot.classList.remove('flap-out');void slot.offsetWidth;slot.classList.add('flap-in')},155);
        },i*55);
      });
    };
    state.timer=setInterval(tick,4200);
  }
  function scan(){document.querySelectorAll('#tripList .trip-flap-icons.has-overflow').forEach(init)}
