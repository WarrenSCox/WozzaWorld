...
(()=>{
  ...
  .trip-stop-dates input[type=date][readonly]{cursor:pointer;padding-left:14px!important;box-sizing:border-box}
...
function wozzaCalendarRender(){const ov=wozzaCalendarEnsure(),view=wozzaCalendarView||new Date(),today=new Date(),rangeMode=wozzaCalendarMode==='range';ov.querySelector('.wozza-calendar-month').textContent=view.toLocaleDateString('en-GB',{month:'long',year:'numeric'});ov.querySelector('.wozza-calendar').classList.toggle('range-mode',rangeMode);ov.querySelector('.wozza-calendar-clear').hidden=rangeMode;ov.querySelector('.wozza-calendar-ok').hidden=rangeMode;ov.querySelector('.wozza-calendar-cancel').textContent=rangeMode?'Close':'Cancel';wozzaCalendarRenderYears(ov,view);const first=new Date(view.getFullYear(),view.getMonth(),1),days=new Date(view.getFullYear(),view.getMonth()+1,0).getDate(),offset=(first.getDay()+6)%7,grid=ov.querySelector('.wozza-calendar-grid');let html='';for(let i=0;i<offset;i++)html+='<span></span>';for(let d=1;d<=days;d++){const x=new Date(view.getFullYear(),view.getMonth(),d),iso=wozzaIsoDate(x),isToday=x.getFullYear()===today.getFullYear()&&x.getMonth()===today.getMonth()&&d===today.getDate(),isStart=rangeMode&&iso===wozzaCalendarRangeStart,isEnd=rangeMode&&iso===wozzaCalendarRangeEnd,isBetween=rangeMode&&wozzaCalendarRangeStart&&wozzaCalendarRangeEnd&&iso>wozzaCalendarRangeStart&&iso<wozzaCalendarRangeEnd;html+=`<button type="button" class="wozza-calendar-day${!rangeMode&&iso===wozzaCalendarSelected?' selected':''}${isToday?' today':''}${isBetween?' trip-range':''}${isStart?' trip-range-start':''}${isEnd?' trip-range-end':''}" data-cal-date="${iso}">${d}</button>`}grid.innerHTML=html;grid.querySelectorAll('[data-cal-date]').forEach(b=>b.onclick=()=>{if(rangeMode)return;wozzaCalendarSelected=b.dataset.calDate;wozzaCalendarRender()})}
function wozzaCalendarOpen(input){if(!input?.isConnected)return;wozzaCalendarMode='edit';wozzaCalendarRangeStart='';wozzaCalendarRangeEnd='';wozzaCalendarTarget=input;const row=input.closest('.trip-destination-stop'),isEnd=input.classList.contains('trip-destination-to'),start=row?.querySelector('.trip-destination-from')?.value||'',existing=input.value||'',seed=existing||(isEnd?start:'')||wozzaIsoDate(new Date()),d=wozzaDateFromIso(seed)||new Date();wozzaCalendarSelected=existing;wozzaCalendarView=new Date(d.getFullYear(),d.getMonth(),1);const ov=wozzaCalendarEnsure();ov.querySelector('.wozza-calendar')?.classList.remove('year-mode');wozzaCalendarRender();if(!ov.open)ov.showModal();requestAnimationFrame(()=>ov.querySelector('.wozza-calendar-cancel')?.focus())}
