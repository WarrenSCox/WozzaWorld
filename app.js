...
function attachListRowEvents(){
  ['visitedCountries','goingCountries','bucketCountries'].forEach(id=>{
    ...
    list.querySelectorAll('[data-bucket-remove]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();openRemoveDialog(b.dataset.bucketRemove,'bucket')});
  });
}
...
function openCountry(c,origin=null){if(c==='Dominican Rep.')c='Dominican Republic';countryCardOrigin=origin||currentCountryCardOrigin();currentCountry=c;renderSheet();$('#countrySheet').classList.add('open');$('#sheetBackdrop').classList.add('open');$('#countrySheet').setAttribute('aria-hidden','false')}
