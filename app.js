...
const TRAVEL_MODES=['Plane','Cruise','Ferry','Train','Car','Campervan','Motorhome','Narrowboat','Coach / Bus','Motorbike','Bicycle','On foot','Other'];
...
function travelModeOptions(selected=''){const raw=String(selected||'').toLowerCase(),normalized=raw==='sea'?'Ferry':raw==='air'?'Plane':selected;return `<option value="">Choose transport...</option>${TRAVEL_MODES.map(t=>`<option value="${t}"${t===normalized?' selected':''}>${t}</option>`).join('')}`}
