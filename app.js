...
;(()=>{
 ...
 .wozza-image-viewer-img{display:block;max-width:94vw;max-height:82vh;width:auto;height:auto;object-fit:contain;border-radius:18px;box-shadow:0 16px 50px rgba(0,35,48,.35);transform-origin:center;will-change:transform;user-select:none;-webkit-user-drag:none;touch-action:none}
 .wozza-image-viewer-caption{position:absolute;left:5vw;right:5vw;bottom:max(22px,env(safe-area-inset-bottom));z-index:3;margin:0;text-align:center;color:#fff;font:600 16px/1.35 sans-serif;text-shadow:0 2px 8px rgba(0,35,48,.55)}
 ...
 function openViewer(img){
  ...
  const stage=document.createElement('div');stage.className='wozza-image-viewer-stage';const big=document.createElement('img');big.className='wozza-image-viewer-img';big.src=img.currentSrc||img.src;big.alt=img.alt||'';big.draggable=false;stage.appendChild(big);const captionText=(img.closest('.country-guide-photo')?.dataset.caption||'').trim();if(captionText){const cap=document.createElement('p');cap.className='wozza-image-viewer-caption';cap.textContent=captionText;viewer.appendChild(cap)}
