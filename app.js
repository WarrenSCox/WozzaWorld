...
$$('.choice-grid button').forEach(b=>b.onclick=()=>{
  ...
  save()
});$$('.header-nav-item').forEach(b=>b.onclick=()=>{$$('.header-nav-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.screen').forEach(x=>x.classList.toggle('active',x.dataset.screen===b.dataset.target));window.scrollTo({top:0,behavior:'smooth'})});$('#sheetClose').onclick=closeSheet;$('#sheetBackdrop').onclick=closeSheet;
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=0.9.1',{updateViaCache:'none'}));
