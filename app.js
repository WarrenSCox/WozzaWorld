...
const oldState=JSON.parse(localStorage.getItem('myworld-state')||'null'),savedState=JSON.parse(localStorage.getItem('wozzaworld-state')||'null');let state=savedState||oldState||structuredClone(seed);state.statuses??={};Object.keys(state.statuses).forEach(c=>{if(state.statuses[c]==='wishlist')state.statuses[c]='bucket'});state.visitHistory??=[];state.companions??={};state.memories??={};state.recycleBin??=[];state.tripRecycleBin??=[];state.companionRecycleBin??=[];state.places??={};state.trips??=[];state.cities??={};state.countryAddedAt??={};state.visitedListPrefs??={sort:'default',year:'all',rating:'all'};state.visitedListPrefs.rating??='all';state.companionBank??=[];state.bucketOrder??=[];state.recycleSelection??=[];state.customDestinations??=[];state.extraStatuses??={};Object.values(state.companions).flat().forEach(n=>{if(n&&!state.companionBank.some(x=>x.toLowerCase()===String(n).toLowerCase()))state.companionBank.push(n)});state.trips.forEach(t=>(t.companions||[]).forEach(n=>{if(n&&!state.companionBank.some(x=>x.toLowerCase()===String(n).toLowerCase()))state.companionBank.push(n)}));
  ...
  ['visitedCountries','goingCountries','bucketCountries'].forEach(id=>{
    ...
    list.querySelectorAll('[data-open-country]').forEach(row=>{
      ...
      if(id!=='bucketCountries'){
        row.addEventListener('pointerdown',e=>{if(e.target.closest('button'))return;longPress=false;startX=e.clientX;startY=e.clientY;timer=setTimeout(()=>{longPress=true;openRemoveDialog(c)},600)});
    ...
    list.querySelectorAll('[data-bucket-remove]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();openRemoveDialog(b.dataset.bucketRemove)});
  });
}
function attachBucketRanking(){
  ...
  if(!document.getElementById('bucket-ranking-style')){
    const style=document.createElement('style');style.id='bucket-ranking-style';style.textContent=`
      ...
      #bucketCountries .bucket-rank{background:none!important;border:0!important;border-radius:0!important;width:30px!important;min-width:30px!important;height:auto!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:flex-start!important;color:#0b1d3b!important;font-size:30px!important;line-height:1!important;font-weight:900!important;box-shadow:none!important}
  ...
  let held=null,timer=null,pointerId=null,startX=0,startY=0,lastY=0,suppressClick=false,manualScroll=false;
  ...
  rows().forEach(row=>{
    ...
    row.addEventListener('pointermove',e=>{
      if(!held){
        ...
        if(Math.hypot(dx,dy)>10){cancelHold();if(Math.abs(dy)>Math.abs(dx)){manualScroll=true;window.scrollBy(0,lastY-e.clientY);lastY=e.clientY;suppressClick=true}}
    ...
    row.addEventListener('pointerup',e=>{if(held){e.preventDefault();e.stopPropagation();finish()}else{cancelHold();if(manualScroll){e.preventDefault();e.stopPropagation();manualScroll=false;setTimeout(()=>{suppressClick=false},80)}}});
...
function tripIsOnHorizon(t){const cs=tripCountries(t);return cs.some(c=>countryHasStatus(c,'going'))||(!cs.length&&t.status==='upcoming')}
...
function renderExtraStats(){const grid=$('.screen[data-screen="me"] .stats-grid');if(!grid)return;const going=$('#meGoing'),trips=$('#meTrips');if(going?.parentElement){const label=going.parentElement.querySelector('span');if(label)label.textContent='Upcoming trips'}if(trips?.parentElement){trips.textContent=state.trips.filter(t=>!tripIsOnHorizon(t)).length;const label=trips.parentElement.querySelector('span');if(label)label.textContent='Trips completed'}grid.querySelectorAll('.wozza-extra-stat').forEach(x=>x.remove());const transportGroup=m=>{m=String(m||'').toLowerCase();if(m==='air')return'By air';if(m==='sea'||m==='narrowboat')return'By water';if(['car','campervan','train','coach / bus','motorbike','bicycle','on foot'].includes(m))return'By land';return m?'Other':''};const modes=state.trips.flatMap(tripTravelModes).map(transportGroup).filter(Boolean);const counts={};modes.forEach(m=>counts[m]=(counts[m]||0)+1);const fav=Object.entries(counts).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]))[0]?.[0]||'';const ratings=state.trips.map(t=>Number(t.rating)||0).filter(Boolean);const avg=ratings.length?ratings.reduce((a,b)=>a+b,0)/ratings.length:0;const modeCard=document.createElement('div');modeCard.className='stat wozza-extra-stat';modeCard.innerHTML=`<strong>${fav?esc(fav):'—'}</strong><span>Favourite way to travel</span>`;const ratingCard=document.createElement('div');ratingCard.className='stat wozza-extra-stat';ratingCard.innerHTML=`<strong>${avg?avg.toFixed(1)+' <em class="avg-rating-star">★</em>':'—'}</strong><span>Average trip rating</span>`;grid.append(modeCard,ratingCard)}
...
async function buildMap(){try{const world=await fetch('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json').then(r=>r.json()),features=topojson.feature(world,world.objects.countries).features;availableCountries=features.map(d=>d.properties.name).filter(Boolean).sort();const svg=d3.select('#worldMap'),projection=d3.geoEqualEarth().fitExtent([[24,28],[976,492]],{type:'Sphere'}),path=d3.geoPath(projection);svg.select('#sphere').attr('d',path({type:'Sphere'}));svg.select('#countries').selectAll('path').data(features).join('path').attr('class','country').attr('d',path).attr('data-country',d=>d.properties.name).attr('tabindex','0').attr('aria-label',d=>d.properties.name);function labelFeature(f){if(f.geometry?.type!=='MultiPolygon')return f;const polys=f.geometry.coordinates.map(coords=>({type:'Feature',properties:f.properties,geometry:{type:'Polygon',coordinates:coords}}));return polys.sort((a,b)=>d3.geoArea(b)-d3.geoArea(a))[0]||f}const labelData=features.map(f=>{const lf=labelFeature(f);return{feature:lf,name:f.properties.name,centroid:path.centroid(lf),bounds:path.bounds(lf)}}).filter(x=>Number.isFinite(x.centroid[0])&&Number.isFinite(x.centroid[1]));const kaliningradPt=projection([20.45,54.70]);if(kaliningradPt)labelData.push({feature:null,name:'Kaliningrad',openName:'Russia',centroid:kaliningradPt,bounds:[[kaliningradPt[0]-24,kaliningradPt[1]-8],[kaliningradPt[0]+24,kaliningradPt[1]+8]],forceLabel:true});const labels=svg.select('#countryLabels').selectAll('text').data(labelData).join('text').attr('class','map-country-label').text(d=>d.name).attr('text-anchor','middle').attr('dominant-baseline','central').style('display','none').attr('tabindex','0').attr('role','button').attr('aria-label',d=>d.name).on('click',(event,d)=>{if(!document.body.classList.contains('map-view'))return;event.preventDefault();event.stopPropagation();openCountry(d.openName||d.name,{type:'map'})}).on('keydown',(event,d)=>{if(!document.body.classList.contains('map-view'))return;if(event.key==='Enter'||event.key===' '){event.preventDefault();openCountry(d.openName||d.name,{type:'map'})}});let portraitLabelBand=0;
function updateCountryLabels(transform){
 ...
 if(!portrait){
  ...
  labels.each(function(d){
   const pt=transform.apply(d.centroid);if(d.name==='Croatia')pt[1]-=15;const bw=(d.bounds[1][0]-d.bounds[0][0])*transform.k,bh=(d.bounds[1][1]-d.bounds[0][1])*transform.k,font=8,tw=Math.max(8,d.name.length*4.25);
   let show=(d.forceLabel||bw>tw*1.25&&bh>font*1.7)&&pt[0]>8&&pt[0]<992&&pt[1]>8&&pt[1]<512;
 ...
 labels.each(function(d){
  const pt=transform.apply(d.centroid);if(d.name==='Croatia')pt[1]-=15;
  const baseW=d.bounds[1][0]-d.bounds[0][0],baseH=d.bounds[1][1]-d.bounds[0][1],baseArea=baseW*baseH;
  const show=d.forceLabel||portraitLabelBand===3||(portraitLabelBand===2&&baseArea>=28)||(portraitLabelBand===1&&baseArea>=115);
  d3.select(this).attr('x',pt[0]).attr('y',pt[1]).style('display',show?null:'none')
 })
...
function openCountry(c,origin=null){countryCardOrigin=origin||currentCountryCardOrigin();currentCountry=c;renderSheet();$('#countrySheet').classList.add('open');$('#sheetBackdrop').classList.add('open');$('#countrySheet').setAttribute('aria-hidden','false')}
...
async function leaveMapMode(){document.body.classList.remove('map-view');resetMapZoom(false);try{screen.orientation?.unlock?.()}catch(e){}try{if(document.fullscreenElement)await document.exitFullscreen()}catch(e){}}
async function showHome(){document.body.classList.remove('trips-view');await leaveMapMode();$$('.header-nav-item').forEach(x=>x.classList.remove('active'));$$('.screen').forEach(x=>x.classList.toggle('active',x.dataset.screen==='home'));countrySlide=0;renderCountryLists();requestAnimationFrame(()=>requestAnimationFrame(()=>setCountrySlide(0,false)));resetMapZoom(false);window.scrollTo({top:0,behavior:'smooth'})}
async function showMap(){document.body.classList.remove('trips-view');document.body.classList.add('map-view','wozza-map-morph-enter');setTimeout(()=>document.body.classList.remove('wozza-map-morph-enter'),720);$$('.header-nav-item').forEach(x=>x.classList.remove('active'));$$('.screen').forEach(x=>x.classList.toggle('active',x.dataset.screen==='home'));window.scrollTo({top:0});try{await screen.orientation?.lock?.('landscape')}catch(e){}}
...
const TRAVEL_MODES=['Air','Car','Campervan','Train','Sea','Narrowboat','Coach / Bus','Motorbike','Bicycle','On foot','Other'];
...
function travelModeOptions(selected=''){return `<option value="">Choose transport...</option>${TRAVEL_MODES.map(t=>`<option value="${t}"${t===selected?' selected':''}>${t}</option>`).join('')}`}
...
function toggleTripCompanions(){const section=document.querySelector('.trip-companions-section'),body=$('#tripCompanionsBody'),b=$('#tripCompanionsToggle');if(!section||!body||!b)return;const next=!section.classList.contains('collapsed');section.classList.toggle('collapsed',next);body.hidden=next;b.textContent=next?'+':'−';b.setAttribute('aria-expanded',String(!next));b.setAttribute('aria-label',next?'Expand travel companions':'Minimise travel companions')}
...
$('#tripForm').onsubmit=e=>{e.preventDefault();const stops=collectDestinationStops(),countries=[...new Set(stops.map(d=>d.country).filter(Boolean))];if(!countries.length){showWozzaAlert('Please choose a country for at least one stop.');return}let name=$('#tripName').value.trim();if(!name)name=countries.length===1?countries[0]:countries.join(' & ');const firstStop=stops[0]||{},lastStop=stops[stops.length-1]||firstStop,start=firstStop.start||'',end=(stops.length===1?firstStop.end:lastStop.end)||'';const companions=[...new Set($$('#tripCompanionBank .companion-tag.selected').map(b=>b.dataset.companion).filter(Boolean))];const existing=editingTripId?state.trips.find(x=>String(x.id)===String(editingTripId)):null,oldCountries=existing?tripCountries(existing).slice():[],t=existing||{id:crypto.randomUUID?.()||String(Date.now())};const tripStatus=existing?.status||'upcoming';Object.assign(t,{name,start,end,countries,destinations:stops,cities:{},companions,plan:existing?.plan||'',todos:collectTripTodos(),notes:$('#tripNotes').value.trim(),rating:Number($('#tripRating').value)||0,status:tripStatus});if(!existing)state.trips.push(t);countries.forEach(c=>{setCountryStatus(c,tripStatus==='upcoming'?'going':'visited',true);if(!state.countryAddedAt[c])state.countryAddedAt[c]=new Date().toISOString();if(tripStatus!=='upcoming'){state.visitHistory=state.visitHistory.filter(x=>!sameCountry(x,c));state.visitHistory.push(c)}});for(const c of oldCountries){if(countries.some(x=>sameCountry(x,c)))continue;const other=state.trips.some(x=>x!==t&&tripCountries(x).some(y=>sameCountry(y,c)));if(!other&&countryHasStatus(c,'going'))setCountryStatus(c,'going',false)}save();if(tripSaveAndContinue){tripSaveAndContinue=false;editingTripId=t.id;setTripDialogMode(true);rememberTripEditorSnapshot();toast(existing?'Trip updated ✓':'Trip created ✓');return}editingTripId=null;tripEditorSnapshot='';$('#tripDialog').close();toast(existing?'Trip updated ✓':'Trip created ✈')};
...
function savePassportName(){const input=$('#passportName');if(!input)return;const value=input.value.trim().replace(/\s+/g,' ').slice(0,24);if(value)localStorage.setItem('wozzaworld-first-name',value);else localStorage.removeItem('wozzaworld-first-name');applyWorldViewName();input.value=value;toast(value?'Stamp name updated ✓':'Stamp name cleared')}
...
function applyWorldViewName(){
  ...
  if(sig){sig.style.display='flex';const x=sig.querySelector('.stamp-name');if(x){const label=n?n.toUpperCase()+"'S":'';x.textContent=label;x.dataset.fullName=label;x.removeAttribute('title');sig.dataset.nameLength=String(label.length);fitWorldViewStampName()}}
  const title=document.getElementById('mapViewTitle');if(title)title.textContent='MY WORLD VIEW';
}
...
.trip-editor-date-range{display:flex!important;align-items:center;gap:12px;flex-wrap:wrap}.trip-editor-transport-icons{display:inline-flex;align-items:center;gap:7px}.trip-editor-transport-icons .travel-mode-icon,.single-stop-summary-mode .travel-mode-icon{width:24px;height:24px;object-fit:contain;display:block}
