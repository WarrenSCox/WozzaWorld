...
function tripCountries(t){return t.countries||[t.country].filter(Boolean)}function orderedTripDates(t){const stops=(t.destinations||[]).filter(Boolean);if(stops.length){const first=stops[0]||{},last=stops[stops.length-1]||{};return {start:first.start||'',end:(stops.length===1?first.end:last.end)||''}}return {start:t.start||'',end:t.end||''}}function tripStars(t){const r=Math.max(0,Math.min(5,Number(t.rating)||0));return `<div class="trip-stars" data-trip-rating="${esc(t.id||'')}" aria-label="Rate ${esc(t.name)}">${[1,2,3,4,5].map(n=>`<button type="button" data-rate="${n}" aria-label="${n} star${n===1?'':'s'}">${n<=r?'★':'☆'}</button>`).join('')}</div>`}function travelModeIcon(mode){const m=String(mode||'').toLowerCase();const icons={air:'air',plane:'air',sea:'sea',ferry:'sea',cruise:'sea',train:'train',car:'car',campervan:'campervan',motorhome:'campervan',narrowboat:'narrowboat',motorbike:'motorbike',bicycle:'bicycle','on foot':'on-foot',other:'other'};const icon=icons[m]||(m.includes('coach')||m.includes('bus')?'coach-bus':'');return icon?`<img class="travel-mode-icon" src="${icon}.png" alt="" aria-hidden="true">`:''}function tripTravelModes(t){const modes=[...(t.destinations||[]).map(d=>d.travelMode),t.travelMode].filter(Boolean);return [...new Set(modes)].filter(m=>travelModeIcon(m))}function tripTravelIcons(t){const modes=tripTravelModes(t);return modes.length?`<div class="trip-travel-icons" aria-label="Travel methods: ${esc(modes.join(', '))}">${modes.map(m=>`<span title="${esc(m)}">${travelModeIcon(m)}</span>`).join('')}</div>`:''}function vibeIcon(vibe){const k=String(vibe||'').toLowerCase(),icons={'city break':'vibe-city.png','beach holiday':'vibe-beach.png','spa & wellness':'vibe-spa-wellness.png','adventure':'vibe-adventure.png','road trip':'vibe-road-trip.png','snow & ski':'vibe-snow-ski.png','cruise':'sea.png','visiting friends & family':'vibe-friends-family.png','celebration':'vibe-celebration.png','great outdoors':'vibe-great-outdoors.png','camping':'vibe-camping.png'},classes={'city break':'city','beach holiday':'beach','spa & wellness':'spa-wellness','adventure':'adventure','road trip':'road-trip','snow & ski':'snow-ski','cruise':'cruise','visiting friends & family':'friends-family','celebration':'celebration','great outdoors':'great-outdoors','camping':'camping'},icon=icons[k];return icon?`<img class="vibe-icon vibe-icon--${classes[k]||'default'}" src="${icon}" alt="" aria-hidden="true">`:''}function tripVibeIcons(t){const travelModes=tripTravelModes(t).map(m=>String(m).toLowerCase()),vibes=[...new Set(t.vibes||[])].filter(v=>vibeIcon(v)&&!(String(v).toLowerCase()==='cruise'&&travelModes.includes('cruise')));return vibes.length?`<div class="trip-vibe-icons" aria-label="Trip vibes: ${esc(vibes.join(', '))}">${vibes.map(v=>`<span title="${esc(v)}">${vibeIcon(v)}</span>`).join('')}</div>`:''}function tripCard(t){const cs=tripCountries(t),td=orderedTripDates(t),dates=td.start?`${pretty(td.start)}${td.end?' – '+pretty(td.end):''}`:'Dates to be confirmed',days=countdownDays(td.start),count=days===null&&tripIsOnHorizon(t)?'PLANNING':days===0?'TODAY ✈':days>0?`${days} DAYS TO GO`:'';const countryFlags=cs.map(c=>`<button type="button" class="trip-country-flag" data-trip-country="${esc(c)}" aria-label="Open ${esc(c)} country card">${flagMarkup(c,'trip-country-flag-img')}</button>`).join('');const countries=cs.length?`<div class="trip-card-country">${cs.map(esc).join(' · ')}</div>`:'';const dateLine=dates?`<div class="trip-card-date">${dates}</div>`:'';return `<div class="trip-card premium-trip editable-trip" data-open-trip="${esc(t.id||'')}" role="button" tabindex="0" aria-label="Open ${esc(t.name)} trip"><div class="trip-card-copy"><div class="trip-card-title-row"><strong class="${String(t.name||'').length>32?'trip-title-long ':''}${String(t.name||'').length>42?'trip-title-xlong':''}">${esc(t.name)}</strong></div>${countries}${dateLine}<div class="trip-card-meta-row">${tripStars(t)}${tripTravelIcons(t)}${tripVibeIcons(t)}</div></div><div class="trip-card-side">${count?`<span class="countdown-badge">${count}</span>`:''}<div class="trip-card-flags" aria-label="Trip countries">${countryFlags}</div></div></div>`}function fitTripCardMetaOverflow(root=document){root.querySelectorAll?.('.trip-card-meta-row').forEach(row=>{const oldTimer=Number(row.dataset.iconSlideTimer||0);if(oldTimer)clearInterval(oldTimer);delete row.dataset.iconSlideTimer;const oldViewport=row.querySelector('.trip-icon-viewport');if(oldViewport){const travel=row.querySelector('.trip-travel-icons'),vibes=row.querySelector('.trip-vibe-icons');[...oldViewport.querySelectorAll('[data-icon-kind="travel"]')].forEach(x=>{x.removeAttribute('data-icon-kind');travel?.appendChild(x)});[...oldViewport.querySelectorAll('[data-icon-kind="vibe"]')].forEach(x=>{x.removeAttribute('data-icon-kind');vibes?.appendChild(x)});oldViewport.remove()}const card=row.closest('[data-open-trip]');if(!card)return;const travel=row.querySelector('.trip-travel-icons'),vibes=row.querySelector('.trip-vibe-icons'),travelItems=[...(travel?.children||[])],vibeItems=[...(vibes?.children||[])],items=[...travelItems,...vibeItems];items.forEach(x=>x.hidden=false);if(!items.length)return;const side=card.querySelector('.trip-card-side'),stars=row.querySelector('.trip-stars');if(!side||!stars)return;const cardBox=card.getBoundingClientRect(),sideBox=side.getBoundingClientRect(),starsBox=stars.getBoundingClientRect();const available=Math.max(0,Math.min(sideBox.left-10,cardBox.right-14)-(starsBox.right+12));if(available<28)return;const gap=7,slotWidth=31,total=items.length*24+gap*Math.max(0,items.length-1);if(total<=available)return;const visibleCount=Math.max(1,Math.floor((available+gap)/slotWidth));if(items.length<=visibleCount)return;const viewport=document.createElement('div');viewport.className='trip-icon-viewport trip-icon-solari-viewport';viewport.style.width=`${Math.min(available,visibleCount*slotWidth-gap)}px`;viewport.setAttribute('aria-label','Transport and trip vibe icons');const track=document.createElement('div');track.className='trip-icon-track trip-icon-solari-track';viewport.appendChild(track);const travelCount=travelItems.length;const source=items.map((x,i)=>{x.dataset.iconKind=i<travelCount?'travel':'vibe';return x});source.forEach(x=>x.remove());if(travel)travel.hidden=true;if(vibes)vibes.hidden=true;const slots=[];for(let i=0;i<visibleCount;i++){const slot=document.createElement('span');slot.className='trip-icon-solari-slot';const face=document.createElement('span');face.className='trip-icon-solari-face';slot.appendChild(face);track.appendChild(slot);slots.push(face)}const cloneInto=(face,node)=>face.replaceChildren(node.cloneNode(true));let offset=0;slots.forEach((face,i)=>cloneInto(face,source[i%source.length]));const advance=()=>{offset=(offset+visibleCount)%source.length;slots.forEach((face,i)=>{const node=source[(offset+i)%source.length];setTimeout(()=>{face.classList.remove('is-flipping');void face.offsetWidth;face.classList.add('is-flipping');setTimeout(()=>cloneInto(face,node),170)},i*90)})};row.appendChild(viewport);const timer=setInterval(advance,3600);row.dataset.iconSlideTimer=String(timer)})}
function scheduleTripCardMetaOverflow(root=document){requestAnimationFrame(()=>requestAnimationFrame(()=>fitTripCardMetaOverflow(root)))}
window.addEventListener('resize',()=>scheduleTripCardMetaOverflow(document));
function countdownDays(d){if(!d)return null;const today=new Date();today.setHours(0,0,0,0);const target=new Date(d+'T00:00:00');return Math.ceil((target-today)/86400000)}
...
.trip-card-meta-row{min-width:0!important;flex-wrap:nowrap!important;overflow:visible!important}.trip-card-meta-row .trip-travel-icons,.trip-card-meta-row .trip-vibe-icons{display:inline-flex!important;align-items:center!important;gap:7px!important;flex:0 0 auto!important;white-space:nowrap!important}.trip-card-meta-row .trip-travel-icons>span,.trip-card-meta-row .trip-vibe-icons>span{flex:0 0 auto!important}.trip-card-meta-row [hidden]{display:none!important}.trip-icon-viewport{height:30px;overflow:visible;display:inline-flex;align-items:center;flex:0 0 auto;margin-left:0}.trip-icon-track{display:inline-flex;align-items:center;gap:7px;white-space:nowrap}.trip-icon-solari-slot{position:relative;width:24px;height:28px;display:inline-flex;align-items:center;justify-content:center;perspective:110px;flex:0 0 24px}.trip-icon-solari-slot::after{content:"";position:absolute;left:1px;right:1px;top:50%;height:1px;background:rgba(16,18,17,.2);opacity:0;pointer-events:none}.trip-icon-solari-face{position:relative;width:24px;height:24px;display:inline-flex;align-items:center;justify-content:center;transform-style:preserve-3d;transform-origin:50% 50%;backface-visibility:hidden}.trip-icon-solari-face>span{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px}.trip-icon-track .travel-mode-icon,.trip-icon-track .vibe-icon{width:24px;height:24px;object-fit:contain;display:block}.trip-icon-solari-face.is-flipping{animation:wozzaSolariFlip .46s cubic-bezier(.22,.7,.25,1) both}.trip-icon-solari-face.is-flipping+.trip-icon-solari-slot::after{opacity:1}@keyframes wozzaSolariFlip{0%{transform:rotateX(0deg) scaleY(1)}45%{transform:rotateX(-90deg) scaleY(.9);opacity:.45}55%{transform:rotateX(88deg) scaleY(.9);opacity:.45}100%{transform:rotateX(0deg) scaleY(1);opacity:1}}@media (prefers-reduced-motion:reduce){.trip-icon-solari-face.is-flipping{animation:none!important}}
/* Per-vibe optical alignment: preserve common 24px slots while visually centring each silhouette. */
