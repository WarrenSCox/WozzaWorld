...
function wozzaCalendarOpen(input){if(!input?.isConnected)return;wozzaCalendarTarget=input;const row=input.closest('.trip-destination-stop'),isEnd=input.classList.contains('trip-destination-to'),start=row?.querySelector('.trip-destination-from')?.value||'',existing=input.value||'',seed=existing||(isEnd?start:'')||wozzaIsoDate(new Date()),d=wozzaDateFromIso(seed)||new Date();wozzaCalendarSelected=existing;wozzaCalendarView=new Date(d.getFullYear(),d.getMonth(),1);const ov=wozzaCalendarEnsure();wozzaCalendarRender();if(!ov.open)ov.showModal();requestAnimationFrame(()=>ov.querySelector('.wozza-calendar-cancel')?.focus())}
