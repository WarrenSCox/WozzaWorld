...
function itineraryIcon(item){if(item.customIcon)return item.customIcon;return ITINERARY_CATEGORIES.find(x=>x[0]===item.category)?.[1]||'📍'}
function itineraryWhen(item){const d=item.startDate?pretty(item.startDate):'Anytime',t=item.timing==='part'?item.partOfDay:(item.startTime||'');return [d,t].filter(Boolean).join(' · ')}
...
function itineraryDialog(){let d=document.getElementById('stopItineraryDialog');if(d)return d;d=document.createElement('dialog');d.id='stopItineraryDialog';d.className='stop-itinerary-dialog';d.innerHTML=`<form method="dialog" class="stop-itinerary-form"><div class="stop-itinerary-head"><div><small>STOP ITINERARY</small><h2 id="stopItineraryHeading">Add activity</h2></div><button type="button" class="stop-itinerary-close" aria-label="Close">×</button></div><label>Activity name<input id="itinName" maxlength="80" placeholder="e.g. Dinner at New York Café"></label><div class="itin-two"><label>Type<select id="itinCategory">${ITINERARY_CATEGORIES.map(x=>`<option>${x[0]}</option>`).join('')}<option>Custom</option></select></label><label>Custom icon<input id="itinCustomIcon" maxlength="4" placeholder="✏️"></label></div><div class="itin-timing"><label>Timing<select id="itinTiming"><option value="fixed">Fixed time</option><option value="part">Part of day</option><option value="anytime">Anytime</option></select></label><label class="itin-part">Part of day<select id="itinPart"><option>Morning</option><option>Afternoon</option><option>Evening</option></select></label></div><div class="itin-two"><label>Start date<input id="itinStartDate" type="date"></label><label class="itin-fixed">Start time<input id="itinStartTime" type="time"></label></div><div class="itin-two itin-fixed"><label>End date<input id="itinEndDate" type="date"></label><label>End time<input id="itinEndTime" type="time"></label></div><label>Location<input id="itinLocation" placeholder="Venue or address"></label><label>Maps / location link<input id="itinLocationUrl" type="url" placeholder="https://…"></label><label>Additional link<input id="itinUrl" type="url" placeholder="Venue, tickets, website…"></label><div class="itin-two"><label>Cost per person<input id="itinCost" inputmode="decimal" placeholder="e.g. £28"></label><label>Booking status<select id="itinBooking"><option>Not needed</option><option>Needed</option><option>Booked</option></select></label></div><label>Booking reference<input id="itinBookingRef" placeholder="Optional"></label><label>Notes<textarea id="itinNotes" placeholder="Anything useful for this activity…"></textarea></label><label class="itin-link-check"><input id="itinLinkNotes" type="checkbox"> Also add these notes to the overall trip Notes</label><label>To do<input id="itinTodo" placeholder="e.g. Download tickets"></label><label class="itin-link-check"><input id="itinLinkTodo" type="checkbox" checked> Also add this to the overall trip To Do List</label><div class="stop-itinerary-actions"><button type="button" id="itinDelete" class="itin-delete">Delete</button><button type="button" class="itin-cancel">Cancel</button><button type="submit" class="primary">Save activity</button></div></form>`;document.body.appendChild(d);d.querySelector('.stop-itinerary-close').onclick=()=>d.close();d.querySelector('.itin-cancel').onclick=()=>d.close();d.querySelector('#itinTiming').onchange=()=>syncItineraryTiming(d);d.addEventListener('click',e=>{if(e.target===d)d.close()});return d}
function syncItineraryTiming(d=itineraryDialog()){const v=d.querySelector('#itinTiming').value;d.querySelectorAll('.itin-fixed').forEach(x=>x.hidden=v!=='fixed');d.querySelectorAll('.itin-part').forEach(x=>x.hidden=v!=='part')}
let activeItineraryRow=null,activeItineraryId='';
function openStopItinerary(row,id=''){activeItineraryRow=row;activeItineraryId=id;const d=itineraryDialog(),items=itineraryItemsForRow(row),x=items.find(i=>String(i.id)===String(id))||{};d.querySelector('#stopItineraryHeading').textContent=id?'Edit activity':'Add activity';const vals={itinName:x.name||'',itinCategory:x.category||'Food & drink',itinCustomIcon:x.customIcon||'',itinTiming:x.timing||'fixed',itinPart:x.partOfDay||'Morning',itinStartDate:x.startDate||row.querySelector('.trip-destination-from')?.value||'',itinStartTime:x.startTime||'',itinEndDate:x.endDate||x.startDate||'',itinEndTime:x.endTime||'',itinLocation:x.location||'',itinLocationUrl:x.locationUrl||'',itinUrl:x.url||'',itinCost:x.cost||'',itinBooking:x.booking||'Not needed',itinBookingRef:x.bookingRef||'',itinNotes:x.notes||'',itinTodo:x.todo||''};Object.entries(vals).forEach(([k,v])=>{const el=d.querySelector('#'+k);if(el)el.value=v});d.querySelector('#itinLinkNotes').checked=false;d.querySelector('#itinLinkTodo').checked=true;d.querySelector('#itinDelete').hidden=!id;syncItineraryTiming(d);d.querySelector('#itinDelete').onclick=()=>{if(!activeItineraryRow)return;setItineraryItemsForRow(activeItineraryRow,itineraryItemsForRow(activeItineraryRow).filter(i=>String(i.id)!==String(activeItineraryId)));d.close()};d.onsubmit=e=>{e.preventDefault();saveStopItinerary()};d.showModal();requestAnimationFrame(()=>d.querySelector('#itinName')?.focus())}
function saveStopItinerary(){const d=itineraryDialog(),row=activeItineraryRow;if(!row)return;const q=id=>d.querySelector('#'+id),name=q('itinName').value.trim();if(!name){q('itinName').focus();return}const x={id:activeItineraryId||crypto.randomUUID?.()||`itin-${Date.now()}`,name,category:q('itinCategory').value,customIcon:q('itinCustomIcon').value.trim(),timing:q('itinTiming').value,partOfDay:q('itinPart').value,startDate:q('itinStartDate').value,startTime:q('itinStartTime').value,endDate:q('itinEndDate').value,endTime:q('itinEndTime').value,location:q('itinLocation').value.trim(),locationUrl:q('itinLocationUrl').value.trim(),url:q('itinUrl').value.trim(),cost:q('itinCost').value.trim(),booking:q('itinBooking').value,bookingRef:q('itinBookingRef').value.trim(),notes:q('itinNotes').value.trim(),todo:q('itinTodo').value.trim()};let items=itineraryItemsForRow(row),at=items.findIndex(i=>String(i.id)===String(x.id));if(at>=0)items[at]=x;else items.push(x);setItineraryItemsForRow(row,items);if(q('itinLinkNotes').checked&&x.notes){const notes=$('#tripNotes'),prefix=`${itineraryIcon(x)} ${x.name}: ${x.notes}`;if(notes&&!notes.value.includes(prefix)){notes.value=(notes.value.trim()?notes.value.trim()+'\n':'')+prefix;updateTripNotesSummary()}}if(q('itinLinkTodo').checked&&x.todo){const exists=collectTripTodos().some(t=>t.text.toLowerCase()===x.todo.toLowerCase());if(!exists){addTripTodoRow({text:x.todo,done:false});updateTripTodoSummary()}}d.close()}
